defmodule Xain do
  ...
  for tag <- @html5_elements -- @self_closing_elements do
    defmacro unquote(tag)(contents \\ "", attrs \\ nil, inner \\ nil) do
      ...
      quote location: :keep,
            do: tag(unquote(tag), unquote(contents), unquote(attrs), unquote(inner), false)
    end
  end
  ...
  for tag <- @self_closing_elements do
    defmacro unquote(tag)(contents \\ "", attrs \\ nil, inner \\ nil) do
      ...
      quote location: :keep,
            do: tag(unquote(tag), unquote(contents), unquote(attrs), unquote(inner), true)
    end
  end
  ...
  defp handle_inner_list([], acc) do
    quoted_join =
      {{:., [], [{:__aliases__, [alias: false], [:Xain]}, :join]}, [],
       [{:xain_buffer, [], Elixir}]}
  ...
  defp handle_inner_list([line | tail], acc) do
    case line do
      ...
      _ ->
        line =
          {:=, [], [{:xain_buffer, [], Elixir}, [{:|, [], [line, {:xain_buffer, [], Elixir}]}]]}
  ...
  defp merge_attrs(content, attrs, tag_name) do
    ...
    attrs_html =
      for {key, val} <- attrs, into: "", do: " #{key}=#{quote_symbol()}#{val}#{quote_symbol()}"
