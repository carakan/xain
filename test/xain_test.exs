defmodule XainTest do
  ...
  test "Example form" do
    expected =
      "<form method=\"post\" action=\"/model\" name=\"form\">" <>
  ...
  test "li, label, and input" do
    expected =
      "<li class=\"string input optional stringish\" id=\"contact_first_name_input\">" <>
  ...
  test "support raw safe content" do
    result =
      markup do
        div ".another" do
          raw({:safe, ["<span>", "my span<script>alert</script>", "</span>"]})
        end
      end

    assert result == ~s(<div class="another"><span>my spanalert</span></div>)
  end
